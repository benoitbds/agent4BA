...
function EditModal({ open, node, onSave, onCancel }: EditModalProps) {
  return (
    <Modal open={open} title={`Modifier ${node.level}`} onClose={onCancel}>
      <NodeForm
        title={node.title}
  ...
  return (
    <li>
      <div className="flex items-center gap-1 px-2 py-1">
        ...
        <span onDoubleClick={() => editable && setEditOpen(true)}>
          {node.title}
        </span>
      ...
      {editable && (
        <EditModal
          ...
          onSave={(vals) => {
            update(node.project_id, { ...node, ...vals });
            setEditOpen(false);
          }}
        />
      )}
