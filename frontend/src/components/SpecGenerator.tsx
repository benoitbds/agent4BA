...
export default function SpecGenerator({ projectId, onGenerated }: Props) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Assuming 'data' is the structured JSON from the backend.
        // The backend should return {"epics": Epic[]}
        setSpecs(data.epics || []);
  ...
  const handleSaveSpecifications = async () => {
    ...
    const requestBody = {
      epics: specs, // specs is already Epic[]
      requirement_title: `AI Specs for ${projectName || 'Project'}`, // Use project name if available
